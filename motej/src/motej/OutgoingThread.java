...
import motej.ext.alt.java.util.concurrent.ConcurrentLinkedQueue;
...
class OutgoingThread extends Thread {
		...
		protected OutgoingThread(String btaddress) throws IOException, InterruptedException {
			...
			outgoing = (L2CAPConnection) Connector.open("btl2cap://"
					+ btaddress
					+ ":11;authenticate=false;encrypt=false;master=false;TransmitMTU=512",
					Connector.WRITE);
			...
			while (active) {
				try {
					...
					if (requestQueue.peek() != null) {
						MoteRequest request = (MoteRequest)requestQueue.poll();
