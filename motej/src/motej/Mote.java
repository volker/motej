...
import motej.ext.alt.javax.swing.event.EventListenerList;
...
//				incoming.join(5000l);
	...
	protected void fireAccelerometerEvent(int x, int y, int z) {
		Object[] listeners = listenerList.getListeners(AccelerometerListener.class);
		AccelerometerEvent evt = new AccelerometerEvent(this, x, y, z);
		for (int i = 0; i < listeners.length; i++) {
			AccelerometerListener l = (AccelerometerListener)listeners[i];
			l.accelerometerChanged(evt);
		}
	}

	protected void fireCoreButtonEvent(int modifiers) {
		Object[] listeners = listenerList.getListeners(CoreButtonListener.class);
		CoreButtonEvent evt = new CoreButtonEvent(this, modifiers);
		for (int i = 0; i < listeners.length; i++) {
			CoreButtonListener l = (CoreButtonListener)listeners[i];
			l.buttonPressed(evt);
		}
	}

	protected void fireIrCameraEvent(IrCameraMode mode, int camera, int x, int y, int size) {
		Object[] listeners = listenerList.getListeners(IrCameraListener.class);
		IrCameraEvent evt = new IrCameraEvent(this, mode, camera, x, y, size);
		for (int i = 0; i < listeners.length; i++) {
			IrCameraListener l = (IrCameraListener)listeners[i];
	...
	protected void fireReadDataEvent(byte[] address, byte[] payload, int error) {
		if (calibrationDataReport == null && error == 0 && address[0] == 0x00 && address[1] == 0x20) {
			...
			Object[] listeners = listenerList.getListeners(DataListener.class);
			DataEvent evt = new DataEvent(address, payload, error);
			for (int i = 0; i < listeners.length; i++) {
				DataListener l = (DataListener)listeners[i];
				l.dataRead(evt);
			}
		}
	}
	...
	protected void fireStatusInformationChangedEvent(StatusInformationReport report) {
		...
		Object[] listeners = listenerList.getListeners(StatusInformationListener.class);
		for (int i = 0; i < listeners.length; i++) {
			StatusInformationListener l = (StatusInformationListener)listeners[i];
